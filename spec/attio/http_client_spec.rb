  ...
  describe "error handling" do
    ...
    context "when connection fails" do
      it "raises ConnectionError" do
        ...
        expect { client.get(path) }.to raise_error(Attio::HttpClient::ConnectionError, "Connection failed: Connection refused")
      end
    end
    ...
    context "when authentication fails" do
      it "raises AuthenticationError" do
        ...
        allow(response).to receive(:body).and_return('{"error":"Invalid API key"}')
    ...
    context "when validation fails" do
      it "raises ValidationError" do
        ...
        allow(response).to receive(:body).and_return('{"error":"Validation failed"}')
    ...
    context "when unexpected status code" do
      it "raises generic Error" do
        ...
        allow(response).to receive(:body).and_return('{"error":"I am a teapot"}')
    ...
    context "when error response has non-JSON body" do
      it "uses body as error message" do
        allow(response).to receive(:code).and_return(500)
        allow(response).to receive(:body).and_return("Plain text error")
